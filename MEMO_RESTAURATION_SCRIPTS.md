...
par accident dans le site Premiere specialite.
...
```text
240e2cb - Sauvegarder les scripts de publication utiles
```
...
```bash
cd ~/ENSEIGNEMENT/SITES_INTERNET/maths-premiere-specialite
git restore --source 240e2cb -- scripts scripts_old
```
...
Remarque : dans le terminal, Python peut afficher `Première spécialité` avec
les accents. C'est normal.
