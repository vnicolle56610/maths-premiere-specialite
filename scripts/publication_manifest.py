...
def bootstrap_manifest_from_auto_docs(
    ...
    for resource in resources:
        ...
        if not published:
            ...
            section_text = section_pages_cache[section_page]
            # Certaines pages de section ne sont jamais régénérées par
            # l'outil (jamais de marqueurs AUTO-DOCS) et pourtant listent
            # déjà des documents à la main : dans ce cas, chercher dans
            # la page entière plutôt que renoncer.
            if AUTO_DOCS_START in section_text and AUTO_DOCS_END in section_text:
                published = contains_in_auto_docs_zone(
                    section_text, resource.destination.name
                )
            else:
                published = resource.destination.name in section_text
